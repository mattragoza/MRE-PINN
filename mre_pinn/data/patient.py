...
class PatientCohort(object):
    ...
    Args:
        nifti_root: Directory to look for NIFTI files.
        patient_ids: 
    ...
    def __init__(
        self,
        nifti_root='/ocean/projects/asc170022p/shared/Data/MRE/MRE_DICOM_7-31-19/NIFTI',
        patient_ids='*',
        sequences=SEQUENCES,
        ...
        self.nifti_root = pathlib.Path(nifti_root)

        if isinstance(patient_ids, str): # glob pattern
            found_patients, patient_ids = self.find_patients(patient_ids, sequences)
            assert found_patients, \
                'No patients matching {patient_ids} with sequences {sequences}'
            self.patient_ids = patient_ids
            self.patients = found_patients

        else: # list of expected patient ids
            patients, missing_patient_ids = self.get_patients(patient_ids, sequences)
            assert not missing_patient_ids, \
    ...
    def find_patients(self, pattern='*', sequences='*'):
        ...
        for patient_dir in sorted(self.nifti_root.glob(pattern)):
            nifti_dir, pid = patient_dir.parent, patient_dir.stem
            try:
    ...
    def get_patients(self, patient_ids, sequences='*'):
        ...
        '''
        patients, avail_patient_ids = self.find_patients(sequences=sequences)
        return set(patient_ids) - set(avail_patient_ids)
